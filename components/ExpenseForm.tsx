...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ expense, onSubmit }) => {
    const { register, handleSubmit, control, formState: { errors, isSubmitting }, watch } = useForm<ExpenseFormData>({
                 ...
                 {fields.map((field, index) => (
                    <div key={field.id} className="flex items-center gap-4 mb-4">
                        <Controller
                            ...
                            render={({ field }) => (
                                <select {...field} className="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[var(--color-secondary-accent)]">
