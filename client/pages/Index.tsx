...
function convertToUSD(amount: number, currency: string) {
  ...
  const initialExpenses: Expense[] = useMemo(
    () => [
      ...
      },
      {
        ...
        status: "approved",
        convertedAmountUSD: convertToUSD(80, "EUR"),
      },
      {
        id: "e3",
        date: new Date(Date.now() - 864e5 * 6).toISOString(),
        description: "Uber",
        category: "Travel",
        amount: 30,
        currency: "GBP",
        ...
        status: "pending",
        convertedAmountUSD: convertToUSD(30, "GBP"),
      },
    ],
    [],
  );
  ...
  for (const [key, list] of byKey) {
    if (list.length > 1) {
      alerts.push({
        ...
        detail: `Matches ${list.length} entries with same description and amount`,
        severity: "medium",
      });
    }
  }
  for (const e of expenses) {
    if (e.amount > 1000)
      alerts.push({
        ...
        detail: `${e.description} at ${e.amount} ${e.currency}`,
        severity: "high",
      });
  }
  return alerts;
...
export default function Index() {
  ...
  useEffect(() => {
    setAlerts(detectFraud(expenses));
  }, [expenses]);
  ...
  const trendData = useMemo(() => {
    const days = [...Array(30)].map(
      (_, i) => new Date(Date.now() - (29 - i) * 864e5),
    );
    return days.map((d) => {
  ...
  return (
    <main>
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="md:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Activity className="size-5 text-primary" /> Last 30 days
              </CardTitle>
              <CardDescription>
                Expense trend in USD (auto-converted)
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="h-64">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <BarChart3 className="size-5 text-primary" /> Summary
              </CardTitle>
                ...
                {group.members.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={
                          b >= 0 ? "text-emerald-600" : "text-rose-600"
                        }
                      >
                        {b >= 0 ? "+$" + b : "-$" + Math.abs(b)}
                      </span>
        ...
        >
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <FileText className="size-5 text-primary" /> Reports & Exports
              </CardTitle>
              <CardDescription>
                Download monthly summaries, tax breakdowns or CSV exports.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <ShieldAlert className="size-5 text-primary" /> Fraud Alerts
              </CardTitle>
              <CardDescription>
                ...
                {alerts.map((a) => (
                  <div key={a.id} className="rounded-md border p-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded ${a.severity === "high" ? "bg-rose-100 text-rose-700" : a.severity === "medium" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
      ...
      <AddExpenseDialog
        ...
        onSubmit={(payload) => {
          const id = `e${Date.now()}`;
          const convertedAmountUSD = convertToUSD(
            payload.amount,
            payload.currency,
          );
          const exp: Expense = {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <p className="mt-4 text-muted-foreground text-lg">
              Create groups, log expenses in real time, get fair splits, settle
              in multiple currencies, and stay protected with anomaly detection.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <Button variant="outline" className="gap-2">
                <Wallet className="size-4" /> Create Group
              </Button>
            </div>
            <div
              id="security"
        ...
        <CardContent>
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-2">
              <span
                className={`text-xs px-2 py-0.5 rounded ${group.enableFraud ? "bg-emerald-100 text-emerald-700" : "bg-muted"}`}
              >
                Fraud detection {group.enableFraud ? "ON" : "OFF"}
              </span>
            </div>
          </div>
          <Separator className="my-4" />
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Invite via email or link. Configure approval workflows.
        </CardDescription>
      </CardHeader>
      <CardContent>
...
function BalancesPanel({
  ...
  function settleSuggestions() {
    const creditors = members
      .map((m) => ({
        id: m.id,
        v: Math.round((balances[m.id] || 0) * 100) / 100,
      }))
      .filter((x) => x.v > 0)
      .sort((a, b) => b.v - a.v);
    ...
    while (i < debtors.length && j < creditors.length) {
      ...
      debtors[i].v += pay;
      creditors[j].v -= pay;
      if (Math.abs(debtors[i].v) < 1e-9) i++;
      if (Math.abs(creditors[j].v) < 1e-9) j++;
    }
    return tx;
  }
          ...
          {tx.map((t, idx) => (
            ...
            >
              <span className="text-sm">
                {members.find((m) => m.id === t.from)?.name} →{" "}
          ...
          <TableBody>
            {expenses.map((e) => (
              <TableRow key={e.id}>
                ...
                <TableCell>
                  {members.find((m) => m.id === e.payerId)?.name}
                </TableCell>
                <TableCell>
                  {e.amount} {e.currency}{" "}
                  <span className="text-muted-foreground">
                    (${e.convertedAmountUSD} USD)
                  </span>
                </TableCell>
                <TableCell>
                  <span
                    className={`text-xs px-2 py-0.5 rounded ${e.status === "approved" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                  >
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a receipt or enter details. OCR extraction can prefill
            fields.
          </DialogDescription>
        </DialogHeader>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="sm:col-span-2">
            ...
            <Input
              ...
              placeholder="e.g., Grocery run, Uber, Utilities"
            />
          </div>
          <div>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              >
                <Camera className="size-4" /> Upload image
              </Button>
              {receipt && (
                <img
                  src={receipt}
