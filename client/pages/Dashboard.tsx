    ...
    async function load() {
      try {
        ...
        setData({
          ...
          settlements: [
            { to: "Sam", amount: 18 },
            { to: "Jordan", amount: 24.35 },
          ],
          alerts: [
            {
              id: "a1",
              title: "Potential duplicate",
              detail: "Matches entry from yesterday",
            },
          ],
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-1">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <TrendingUp className="size-5 text-primary" /> Net Balance
            </CardTitle>
            <CardDescription>Overall position</CardDescription>
          </CardHeader>
          <CardContent>
            <p
              className={`text-3xl font-bold ${data?.net >= 0 ? "text-emerald-500" : "text-rose-400"}`}
            >
              {data ? `$${Math.abs(data.net).toFixed(2)}` : "—"}
            </p>
            <p className="text-sm text-muted-foreground mt-2">
              {data?.net >= 0 ? "You are owed" : "You owe"}
            </p>
          </CardContent>
        </Card>
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Users className="size-5 text-primary" /> Split Distribution
            </CardTitle>
