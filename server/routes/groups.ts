...
interface Group { id: string; name: string; defaultCurrency: string; members: string[]; createdBy: string; }
const groups: Group[] = [];
...
export const createGroup: RequestHandler = (req, res) => {
  ...
  if (!name || !defaultCurrency || !creatorId) return res.status(400).json({ message: "Missing fields" });
  const id = "g" + Math.random().toString(36).slice(2,8);
  const group: Group = { id, name, defaultCurrency, members: [creatorId], createdBy: creatorId };
  groups.push(group);
  res.json({ group });
};
